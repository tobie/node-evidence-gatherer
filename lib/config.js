...
function createConfig(options) {
  ...
  var self = {}, _o = {}, _keys, props = {
    templates: {
      get: function() {
        var templates = _o.templates;
        if (!templates) {
          _o.templates = {
            index: path.join(__dirname, '..', 'templates', 'html5.mustache')
          }
        } else if (Object.prototype.toString.call(templates) === '[object Array]') {
          var ext = self.templateExtension;
          
          _o.templates = templates.reduce(function(output, template) {
            var basename = path.basename(template, ext);
            if (basename === template) {
              template = path.join(self.templatesDir, basename + ext);
            }
    ...
    fixturesDir: {
      get: function() {
        _o.fixturesDir = _o.fixturesDir || path.join(_o.inputDir, 'fixtures');;
        return _o.fixturesDir;
      }
    },
    
    fixtureContentTypes: {
      get: function() {
        _o.fixtureContentTypes = _o.fixtureContentTypes || ['html', 'css', 'js'];
        return _o.fixtureContentTypes;
      }
    },
    
    testFilePattern: {
      get: function() {
        _o.testFilePattern = new RegExp(_o.testFilePattern || "_test\\.js$");
        return _o.testFilePattern;
      }
    },
    
    testFileReplacement: {
      get: function() {
        _o.testFileReplacement = _o.testFileReplacement || '';
        return _o.testFileReplacement;
      }
    },
...
exports.createConfig = function (options, callback) {
  ...
  optionsFromUser(function(err, userOptions) {
    ...
    optionsFromDir(config.inputDir, function(err, dirOptions) {
      if (err) {
        callback(err);
        return;
      }
